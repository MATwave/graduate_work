...
async def persons_all(person_service: PersonService = Depends(get_person_service),
                      commons: CommonQueryParams = Depends(CommonQueryParams),) -> list[APIPerson]:
    """
...
@router.get("/{person_id}", response_model=APIPerson, summary="Поиск персоны по его UUID")
async def person_details(person_id: UUID,
                         person_service: PersonService = Depends(get_person_service)) -> APIPerson:
    """
...
async def person_films(person_id: UUID,
                       ...
                       film_service: FilmService = Depends(get_film_service)) -> list[APIPersonFilms]:
    """
