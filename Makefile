...
admin_panel_up:
	...
	docker exec -T admin_panel bash -c "python manage.py migrate" || \
		(echo "Failed to run migrations" && exit 1)

	# Сбор статических файлов
	@echo "Collecting static files..."
	docker exec -T admin_panel bash -c "python manage.py collectstatic" || \
		(echo "Failed to collect static files" && exit 1)

	# Создание суперпользователя
	@echo "Creating superuser..."
	docker exec -T admin_panel python  manage.py shell -c "from django.contrib.auth import get_user_model; User = get_user_model(); User.objects.create_superuser('admin', 'admin@example.com', 'admin')"  && \
...
admin_panel_fill_db:
	...
	docker exec -T admin_panel bash -c "cd sqlite_to_postgres && python load_data.py && python tests.py" && \
	echo "The data has been successfully migrated."

admin_panel_test_fill:
	@echo "Testing data migration from SQLite to PostgreSQL"
	docker exec -T admin_panel bash -c "cd sqlite_to_postgres && python tests.py"
